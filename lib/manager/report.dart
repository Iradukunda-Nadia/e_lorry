...
class _ReportState extends State<Report> {
  ...
  _postTrip() async {
    ...
    var query = collectionReference.where("timestamp", isEqualTo: DateFormat(' dd MMM yyyy').format(DateTime.now()) );
  ...
  _lpoCheck() async {
    ...
    var query = collectionReference.where("date", isEqualTo: DateFormat(' dd MMM yyyy').format(DateTime.now()) );
    query.getDocuments().then((querySnapshot) {
      if (querySnapshot.documents.length > 0) {
        querySnapshot.documents.forEach((document)
        async {
